...
*/
  ...
  public function calculate() {
    ...
    foreach($this->_group_grades as $group_id => $group_mark) {
      ...
      /* (2)
       * Get the multiplication factor we need to calculate the webgpa scores
      ...
      if (array_key_exists($group_id, $this->_group_members)) {
        ...
        foreach($this->_group_members[$group_id] as $i => $member_id) {
          ...
          /* (3)
           * Get the webgpa score = (total received by a member / average total score if all students equal ) * multiplication-factor
           */

          $avg_total = $this->_group_split100[$group_id] * $q_count;

          $this->_webgpa_scores[$member_id] = ( $total_score / $avg_total) * $multi_factor;

          /* (4)
           * Get the member's intermediate grade = webgpa score * weighted-group-mark   (does not include penalties)
           */

          if (is_array($this->_calc_group_submitters[$group_id])) {
            $intermediate_grade = (($this->_webgpa_scores[$member_id] * $pa_group_mark) + $nonpa_group_mark);
          } else {
